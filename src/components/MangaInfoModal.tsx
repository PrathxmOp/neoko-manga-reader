...
export const MangaInfoModal: React.FC<MangaInfoModalProps> = ({ manga, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-3 pt-14 pb-16 sm:p-4 bg-black/85 backdrop-blur-md animate-fade-in">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-lg bg-[#161327] border border-[#2b2746] rounded-3xl shadow-2xl overflow-hidden max-h-[calc(100vh-130px)] sm:max-h-[85vh] flex flex-col z-10 animate-scale-up my-auto">
        {/* Banner Artwork & Close Button */}
        <div className="relative h-32 sm:h-44 w-full overflow-hidden bg-[#0c0c14] shrink-0">
          <img
          ...
          <button
            onClick={onClose}
            className="absolute top-3 right-3 p-2 rounded-full bg-[#120f23]/80 hover:bg-white/20 text-white/80 hover:text-white border border-white/10 backdrop-blur-md transition-colors z-20"
          >
            <X className="w-4 h-4" />
          </button>

          {/* Manga Header Overlay Content */}
          <div className="absolute bottom-2 left-3.5 right-3.5 flex items-end gap-3 z-10">
            {/* Poster Image */}
            <div className="relative w-16 h-24 sm:w-22 sm:h-30 rounded-xl overflow-hidden shadow-xl border-2 border-[#2b2746] bg-[#0c0c14] shrink-0">
              <img
            ...
            <div className="flex flex-col justify-end min-w-0 pb-0.5">
              ...
              <h2 className="font-display font-extrabold text-sm sm:text-lg text-white line-clamp-2 leading-tight drop-shadow-md">
                {currentManga.title}
              </h2>
              {currentManga.author && (
                <p className="text-[11px] text-[#7c779b] truncate font-medium mt-0.5">
        ...
        <div className="p-3.5 sm:p-5 space-y-3.5 flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-[#2b2746]">
          {/* Genre Tags */}
          {genres.length > 0 && (
          ...
          <div className="grid grid-cols-2 gap-2 bg-[#120f23]/60 p-2.5 rounded-2xl border border-[#2b2746]">
          ...
          <div className="space-y-1.5">
            ...
            {loading ? (
              ...
              <div className="bg-[#120f23]/40 p-3 rounded-2xl border border-[#2b2746]/60">
        ...
        <div className="p-3 sm:p-3.5 bg-[#120f23] border-t border-[#2b2746] flex flex-col sm:flex-row items-center gap-2 shrink-0 z-20 shadow-2xl">
          <button
            onClick={handleReadNow}
            className="w-full sm:flex-1 h-10 sm:h-11 px-4 rounded-2xl bg-gradient-to-r from-[#9d86e9] to-[#7c5cdb] hover:from-[#b19cf5] hover:to-[#8c6de6] text-black font-extrabold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg shadow-[#9d86e9]/20 transition-all active:scale-[0.98]"
          >
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <div className="relative flex-1 sm:flex-initial flex items-center">
              <button
                ...
                className={`h-10 sm:h-11 px-3 sm:px-4 rounded-l-2xl text-xs font-extrabold flex items-center justify-center gap-1.5 border-y border-l transition-all active:scale-[0.98] ${
                  isSaved
              ...
              <select
                ...
                className={`h-10 sm:h-11 px-2 rounded-r-2xl text-xs font-bold border-y border-r outline-none cursor-pointer transition-colors ${
                  isSaved
            ...
            <button
              ...
              className="h-10 sm:h-11 px-3.5 rounded-2xl bg-[#231f3d] hover:bg-[#2e2a4f] text-white border border-[#2b2746] font-extrabold text-xs flex items-center justify-center gap-1.5 transition-all active:scale-[0.98]"
